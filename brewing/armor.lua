...
armor:register_armor("brewing:magic_heart_amulet", {
	...
	on_equip = function(player, index, stack)
		...
		local with_amulet_hp_max = current_hp_max + brewing.heart_amulet_hp_inc_by
		player:set_properties({hp_max = with_amulet_hp_max})
		local player_name = player:get_player_name()
		minetest.chat_send_player(player_name, S("Your max health is increased by").." "..tostring(brewing.heart_amulet_hp_inc_by)..".")
	end,
	on_unequip = function(player, index, stack)
		...
		player:set_properties({hp_max = with_amulet_hp_max})
		local player_name = player:get_player_name()
		minetest.chat_send_player(player_name, S("Your max health is decreased by").." "..tostring(brewing.heart_amulet_hp_inc_by)..".")
	end,
})
...
armor:register_armor("brewing:magic_health_amulet", {
	...
	on_equip = function(player, index, stack)
		...
		if power_level == 0 then
			power_level = brewing.health_amulet_power_level
	...
	on_damage = function(player, index, stack)
		...
		if player_hp > 0 then
			local meta = stack:get_meta()		
			local power_level = meta:get_int("health_amulet_power_level")	
			--minetest.chat_send_player("singleplayer", "going to heal: "..power_level.." "..brewing.health_amulet_heal_points)
			-- Increase the player's hp
			local new_hp = player_hp + brewing.health_amulet_heal_points
			player:set_hp(new_hp)
			--minetest.chat_send_player("singleplayer", S("You has been healed!"))
			-- Decrease the amulet power
			power_level = power_level - brewing.health_amulet_heal_points
			if power_level > 0 then				
...
armor:register_armor("brewing:magic_blue_star_amulet", {
	...
	groups = {armor_amulet=1, armor_use=500, physics_speed=brewing.star_blue_amulet_speedup},
	damage_groups = {cracky=3, snappy=3, choppy=3, crumbly=3, level=1},
})
