...
local formspec =
	...
	"image[0,4;1,1;"..brewing.ignitor["image"].."]"..	
	...
	"label[6,1;".. brewing.filled_flasks .."x]"..
...
local function allow_metadata_inventory_put(pos, listname, index, stack, player)
	...
	elseif listname == "ignitor" then
		...
		if iswater== brewing.ignitor["name"] then
			return stack:get_count()
		else
			return 0
		end		
	elseif listname == "flask" then
		local iswater= stack:get_name()
		if iswater== brewing.flask_name then
			return stack:get_count()
		else
			return 0
		end			
...
local function decrease_stacks(pos, ing_listname, ing_stack, howmuch)
	...
	inv:set_stack(ing_list_name, 1, ing_stack)		
end
		...
		if ignitor_name== brewing.ignitor["name"] and isvalidwater and flask_name== brewing.flask_name and flask_count >= brewing.filled_flasks then
			...
			if brewed ~= nil then
				if inv:room_for_item("dst", brewed) then
					...
					brewed:set_count(brewing.filled_flasks)
					--Make the potion/s!!!
